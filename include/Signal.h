...
public:
    ...
    void DisconnectAll() { mDelegates.clear(); }
...
#define Disconnection(sender, signal, _delegate) (sender).signal.Disconnect(_delegate)


#define CONNECT(sender, signal, receiver,...) (sender).signal.Connect<std::decay_t<decltype(receiver)>, __VA_ARGS__>(receiver)
#define DISCONNECT(sender, signal, _delegate) (sender).signal.Disconnect(_delegate)
#define SELF_CONNECT(signal, ...) CONNECT(*this, signal, *this, __VA_ARGS__)
#define SELF_DISCONNECT(signal, _delegate) DISCONNECT(*this, signal, _delegate)
